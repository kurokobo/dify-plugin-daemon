...
// setEnvVars sets environment variables for testing and returns a function to restore the original values
func setEnvVars(t *testing.T, enabled string, keys string) func() {
	// save original values
	origEnabled := os.Getenv("THIRD_PARTY_SIGNATURE_VERIFICATION_ENABLED")
	origKeys := os.Getenv("THIRD_PARTY_SIGNATURE_VERIFICATION_PUBLIC_KEYS")

	// set new values
	if err := os.Setenv("THIRD_PARTY_SIGNATURE_VERIFICATION_ENABLED", enabled); err != nil {
		t.Fatalf("failed to set THIRD_PARTY_SIGNATURE_VERIFICATION_ENABLED: %s", err.Error())
	}
	if err := os.Setenv("THIRD_PARTY_SIGNATURE_VERIFICATION_PUBLIC_KEYS", keys); err != nil {
		t.Fatalf("failed to set THIRD_PARTY_SIGNATURE_VERIFICATION_PUBLIC_KEYS: %s", err.Error())
	}

	// return function to restore original values
	return func() {
		os.Setenv("THIRD_PARTY_SIGNATURE_VERIFICATION_ENABLED", origEnabled)
		os.Setenv("THIRD_PARTY_SIGNATURE_VERIFICATION_PUBLIC_KEYS", origKeys)
	}
}
...
func TestVerifyPluginWithThirdPartyKeys(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "third-party verification disabled by default (should fail)",
			enabled:       "",
			keyPaths:      "",
			signedDecoder: signedDecoder1,
			expectSuccess: false,
		...
		{
			name:          "third-party verification disabled explicitly (should fail)",
			enabled:       "false",
			keyPaths:      "",
			signedDecoder: signedDecoder1,
			expectSuccess: false,
		...
		{
			name:          "third-party verification disabled with public key 1 (should fail)",
			enabled:       "false",
		...
		{
			name:          "third-party verification disabled with public key 2 (should fail)",
			enabled:       "false",
		...
		{
			name:          "third-party verification disabled with both keys (should fail)",
			enabled:       "false",
		...
		{
			name:          "third-party verification enabled with public key 1 (should succeed)",
			enabled:       "true",
			keyPaths:      publicKey1Path,
			signedDecoder: signedDecoder1,
			expectSuccess: true,
		...
		{
			name:          "third-party verification enabled with public key 2 (should fail)",
			enabled:       "true",
			keyPaths:      publicKey2Path,
			signedDecoder: signedDecoder1,
			expectSuccess: false,
		...
		{
			name:          "third-party verification enabled with both keys (should succeed)",
			enabled:       "true",
			keyPaths:      fmt.Sprintf("%s,%s", publicKey1Path, publicKey2Path),
			signedDecoder: signedDecoder1,
			expectSuccess: true,
		...
		{
			name:          "third-party verification enabled with empty key path (should fail)",
			enabled:       "true",
			keyPaths:      "",
			signedDecoder: signedDecoder1,
			expectSuccess: false,
		...
		{
			name:          "third-party verification enabled with non-existent key path (should fail)",
			enabled:       "true",
		...
		{
			name:          "third-party verification enabled with multiple keys including non-existent path (should fail)",
			enabled:       "true",
		...
		{
			name:          "third-party verification enabled with both keys, for file signed with key 2 (should succeed)",
			enabled:       "true",
			keyPaths:      fmt.Sprintf("%s,%s", publicKey1Path, publicKey2Path),
			signedDecoder: signedDecoder2,
			expectSuccess: true,
		...
		{
			name:          "third-party verification enabled with both keys, for modified file (should fail)",
			enabled:       "true",
			keyPaths:      fmt.Sprintf("%s,%s", publicKey1Path, publicKey2Path),
			signedDecoder: modifiedDecoder1,
			expectSuccess: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			restoreEnv := setEnvVars(t, tt.enabled, tt.keyPaths)
			defer restoreEnv()
